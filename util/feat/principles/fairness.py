...
class Fairness:
    ...
    """
    def __init__(self, model_params, fair_threshold, fair_metric_name, fair_is_pos_label_fav, fair_concern , fair_priority , fair_impact, fair_metric_type, fairness_metric_value_input):
        """
        Parameters
        ------------------
        model_params : list
    ...
    def _get_max_min_group(self, subGrpDf, perf_metric='sample_count'):
        ...
        for ix in subGrpDf.index:
            if subGrpDf.loc[ix]['pos_labels']>=self.min_samples_per_label and  subGrpDf.loc[ix]['neg_labels']>=self.min_samples_per_label:
                best_ix = ix
    ...
    def compile(self, disable=[], n_threads=1):
        ...
        elif self.tradeoff_status == 0 and 'tradeoff' not in self.compile_disable and ('evaluate' not in self.compile_disable or self.compile_disable_map['evaluate']):
            try :
        ...
        #check if tradeoff hasn't run and user wants to skip, print skipped
            ...
            if 'tradeoff' in self.compile_disable or ('evaluate' in self.compile_disable and not self.compile_disable_map['evaluate']):
        ...
        elif self.feature_imp_status == 0 and ('feature_importance' not in self.compile_disable or self.compile_disable_map['feature_importance']) and ('evaluate' not in self.compile_disable or self.compile_disable_map['evaluate']):
            try :
                self.feature_importance(output=False, n_threads=n_threads, disable=list(self.compile_disable_map['feature_importance']))
        ...
        elif self.feature_imp_status == 0 and ('feature_importance' in self.compile_disable or 'evaluate' in self.compile_disable):
            if ('feature_importance' in self.compile_disable and not self.compile_disable_map['feature_importance']) or ('evaluate' in self.compile_disable and not self.compile_disable_map['evaluate']):
                ...
                if model_object is None:
                    ...
                    for var_name in op_name:
                        ...
                        try:
                            ...
                            self.feature_imp_status = -1
                            print("Skipped: Feature importance is skipped due to train_op_name/predict_op_name/predict_proba_op_name error")
                            return
            #to show progress bar
            ...
            if max_workers >=1:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers = max_workers) as executor:
                    ...
                    for i in self.model_params[0].non_intersect_pvars:
                        ...
                        threads.append(executor.submit(Fairness._feature_imp_loo, p_variable=i, use_case_object=use_case_object, fimp_pbar=fimp_pbar, worker_progress=worker_progress, model_object=model_object))
    ...
    def _feature_imp_loo(p_variable, use_case_object, fimp_pbar, worker_progress, model_object):
        ...
        for k in range(len(use_case_object.model_params)):
            ...
            try:
                ...
                if isinstance(x_train, pd.DataFrame):
                    ...
                    train_op(x_train.drop(columns=[p_variable]), y_train)  # train_op_name is string, need to use getattr[] to get the attribute?
                else :
                ...
                print("Skipped: LOCO analysis is skipped for [", p_variable, "] due to x_train/y_train error")
            ...
            if len(pre_y_pred_new.shape) == 1 and pre_y_pred_new.dtype.kind in ['i','O','U']:
                pre_y_pred_new, pos_label2 = use_case_object._check_label(pre_y_pred_new, pos_label, neg_label, obj_in=obj_in, y_pred_flag=True) # Fairness > Usecaseobject provided it is cm/cs
            else:
        ...
        loo_perf_value = use_case_object.perf_metric_obj.translate_metric(use_case_object.perf_metric_name, obj=use_case_object.perf_metric_obj, y_pred_new=y_pred_new, y_prob_new=y_prob_new)
        ...
        for j in use_case_object.model_params[0].non_intersect_pvars:
            ...
            loo_fair_value, loo_priv_m_v = use_case_object.fair_metric_obj.translate_metric(use_case_object.fair_metric_name, obj=use_case_object.fair_metric_obj, y_pred_new=y_pred_new, y_prob_new=y_prob_new)[:2]
    ...
    def _print_evaluate(self):
        ...
        if self.fair_metric_obj.result.get("indiv_fair") is None:
            ...
            for k in self._use_case_metrics["indiv_fair"]:
                ...
                v = "{:.{decimal_pts}f}".format(self.fair_metric_obj.result.get("indiv_fair").get(k),
                                                decimal_pts=self.decimals)
        ...
        print('*Disclaimer: The outcome is calculated based on your inputs and is provided for informational purposes only. Should you decide to act upon the information herein, you do so at your own risk and Veritas Toolkit will not be liable or responsible in any way. ')
        sys.stdout.flush()
    ...
    def _print_tradeoff(self):
        ...
        if self.tradeoff_obj.replacement_flag == 1:
            print("Note: Fairness metric {} is not supported in tradeoff. {} is suggested as a replacement for tradeoff only.".format(self.fair_metric_name, self.tradeoff_obj.fair_metric_name))
    ...
    def _print_feature_importance(self):
        ...
        perf_metric_truncated = (self.perf_metric_name[:23] + '..') if len(self.perf_metric_name) > 25 else self.perf_metric_name
        fair_metric_truncated = (self.fair_metric_name[:23] + '..') if len(self.fair_metric_name) > 25 else self.fair_metric_name
        ...
        for i, i_var in enumerate(self.model_params[0].non_intersect_pvars):
            ...
            print()
            print("-" * 124)
            print("|{:<30}|{:<25}|{:<25}|{:<20}|{:<18}|".format("Removed Protected Variable", perf_metric_truncated,
                                                                fair_metric_truncated, "Fairness Conclusion",
                                                                "Suggestion"))
            print("-" * 124)
            for j in self.model_params[0].non_intersect_pvars:
                col1, col2, col3, col4 = self.feature_imp_values[i_var][j]
                print("|{:<30}|{:<25.{decimal_pts}f}|{:<25.{decimal_pts}f}|{:<20}|{:<18}|".format(j, col1, col2, col3, (col4).title(), decimal_pts=self.decimals))
                print("-" * 124)
            print()
    ...
    def mitigate(self, p_var=[], method=['reweigh'], cr_alpha=1, cr_beta=None, rw_weights=None, transform_x=None, transform_y=None, model_num=0):
        ...
        _input_parameter_no_filtering = {
            ...
            "transform_x": [transform_x, (NoneType, pd.DataFrame, str), exp_transform_x],
        ...
        if 'threshold' in self.mitigate_method:
            ...
            for i in self.mitigate_p_var:
                if i in self.tradeoff_obj.result:
                    ...
                    print("|{:<30}|{:<23.{decimal_pts}f}|{:<23.{decimal_pts}f}|".format(i, th1, th2, decimal_pts=self.decimals))
                    print("-" * 80)
            print()
        ...
        if 'reweigh' in self.mitigate_method:
            ...
            if self.rw_is_transform:
                ...
                for key, value in self.mitigate_result['reweigh'][1].items():
                    ...
                    label = key[n]
                    weight = value
                    print("|{:<30}|{:<23}|{:<23.{decimal_pts}f}|".format(", ".join(str(i) for i in p_var_value), label, weight, decimal_pts=self.decimals))
                    print("-" * 80)
                print()
    ...
    def _correlate(self, p_var, alpha, beta, transform_df):
        ...
        if beta is not None and transform_df is not None:
            ...
            cols_wo_pvar = [col for col in self.model_params[0].x_train.columns if col not in self.model_params[0].non_intersect_pvars] 
            x_train_decorr = pd.DataFrame(x_train_decorr,columns= cols_wo_pvar) #
            x_train_decorr[self.model_params[0].non_intersect_pvars] = self.model_params[0].x_train[self.model_params[0].non_intersect_pvars]
        ...
        cols_wo_pvar = [col for col in self.model_params[0].x_test.columns if col not in self.model_params[0].non_intersect_pvars] 
    ...
    def rootcause(self, p_var=[], multi_class_target=None, model_num=0):
        ...
        _input_parameter_lookup = {
            ...
            "multi_class_target": [multi_class_target, (str, float, int), self.model_params[self.rootcause_model_num].model_object.classes_]
        }
        ...
        for i in p_var:
            ...
            self.rootcause_values[i] = self._rootcause_group_difference(tran_shap_values_pvar, group_mask, feature_names)
        rca_pbar.update(10)
    ...
    def _print_rootcause(self):
        ...
        for i, i_var in enumerate(self.rootcause_values):
            ...
            bar_plot = ax.barh(sorted_labels, sorted(group_diff, reverse=True), align='center', color='#12239E', zorder=2)
            ...
            for bar in bar_plot:
                ...
                ax.text(width, bar.get_y() + bar.get_height()/2, '{:.{decimal_pts}f}%'.format(width*100, decimal_pts=1), ha='left', va='center')
                ...
                def filtering(protected_feature):
                    ...
                    for metric in NewMetric.__subclasses__():
                        if metric.metric_name in result_fairness[protected_feature]['fair_metric_values'].keys():
                            if metric.metric_name != self.fair_metric_name and metric.metric_name in filtered_data.columns:
                    ...
                    with plot_output:
                        ...
                        plt.bar(metrics, values, color=clrs, align='center', width=0.5, bottom=neutral_pos)

                        plt.yticks(fontsize=25)
                        label = [FairnessMetrics.map_fair_metric_to_group.get(metrics[i])[5] for i in range(len(metrics))]
                        wrap_label = []
                        for l in label:
                            l_ = l.split(" ")
                            l_.insert(1, "\n")
    ...
    def _model_type_input(self):
        ...
        for idx, i in enumerate(self.model_params):
            if i.model_type != model_type_exp:
                self.err.push('value_error', var_name="model_params["+str(idx)+"]'s model_type", given=str(i.model_type),
        ...
        if model_type_exp in ['classification','regression'] and len(self.model_params)!=1:
            self.err.push('value_error', var_name="model_type="+model_type_exp+" model_params length", given=len(self.model_params),
                              expected=1,
                              function_name="_model_type_input")
    ...
    def _fairness_tree(self, is_pos_label_favourable = True):
        ...
        if self.fair_metric_type not in ['difference', 'ratio']:
            err_.append(['value_error', "fair_metric_type", str(self.fair_metric_type),str(['difference', 'ratio'])])
    ...
    def _get_confusion_matrix_optimized(self, y_true, y_pred, sample_weight, curr_p_var = None, feature_mask = None):
        ...
        if self._model_type_to_metric_lookup[self.model_params[0].model_type][0] == "classification" :
            ...
            if hasattr(self, '_rejection_inference_flag'):
                rejection_inference_filter = {k: v for k, v in getattr(self, '_rejection_inference_flag').items() if v == True}
            else:
            ...
            if curr_p_var is None:
                ...
                if y_pred is None:
                    ...
                    # privileged group
                    tp_p = np.sum(correct*y_true*mask*sample_weight, axis=2, where=maskFilter)
                    fp_p = np.sum(incorrect*(1-y_true)*mask*sample_weight, axis=2, where=maskFilter)
                    tn_p = np.sum(correct*(1-y_true)*mask*sample_weight, axis=2, where=maskFilter)
                    fn_p = np.sum(incorrect*y_true*mask*sample_weight, axis=2, where=maskFilter)
                    # unprivileged group
                    tp_u = np.sum(correct*y_true*(1-mask)*sample_weight, axis=2, where=maskFilter)
                    fp_u = np.sum(incorrect*(1-y_true)*(1-mask)*sample_weight, axis=2, where=maskFilter)
                    tn_u = np.sum(correct*(1-y_true)*(1-mask)*sample_weight, axis=2, where=maskFilter)
                    fn_u = np.sum(incorrect*y_true*(1-mask)*sample_weight, axis=2, where=maskFilter)

                    if rejection_inference_filter is not None and self._rejection_inference_flag[curr_p_var] == True :
            ...
            if label_size != -1 :
                ...
                if mdl.y_true is not None :
                    if len(mdl.y_true_labels) < label_size :
                        err_.append(["length_error", "y_true label length in model_params["+str(idx)+"]", len(mdl.y_true_labels), '>='+str(label_size)])
                if mdl.y_train is not None :
                    if len(mdl.y_train_labels) < label_size :
                        err_.append(["length_error", "y_train label length in model_params["+str(idx)+"]", len(mdl.y_train_labels), '>='+str(label_size)])
                if mdl.y_pred is not None :
                    if len(mdl.y_pred_labels) < label_size :
                        err_.append(["length_error", "y_pred label length in model_params["+str(idx)+"]", len(mdl.y_pred_labels), '>='+str(label_size)])

            if err_ == []:
                return successMsg
            else:
                for i in range(len(err_)):
                    self.err.push(err_[i][0], var_name=err_[i][1], given=err_[i][2], expected=err_[i][3], function_name="check_label_data_for_model_type")
            self.err.pop()
        ...
        if model_type != "uplift":
            ...
            if model_type == "classification":
                if neg_label is not None:
                    if neg_label_size != 1:
                        err_.append(['length_error', "neg_label binary label", str(neg_label_size), "1"])
                if pos_label_size != 1:
                    err_.append(['length_error', "pos_label binary label", str(pos_label_size), "1"])
        
        if err_ == []:
            return successMsg
        else:
            for i in range(len(err_)):
                err.push(err_[i][0], var_name=err_[i][1], given=err_[i][2], expected=err_[i][3], function_name="_check_binary_restriction")
            err.pop()
    ...
    def _input_parameter_compile_processing(self, disable=[]):
            ...
            for item in disable:
                ...
                if not isinstance(item, str):
                    ...
                    if "|" in item and ">" not in item:
                        err_.append(['value_error', "values in disable", item, "string to contain '>' if it contains '|' pipe"])
                    # Check that it should only contain "|" or ">" symbols but not any other symbols.
                        ...
                        err_.append(['value_error', "values in disable", item, "string to contain '|' and/or '>' characters only"])
                    else:
                        # Check that there should only be 1 ">" if present
                        if item.count(">") > 1:
                            err_.append(['value_error', "values in disable", item, "string to contain only one '>'"])
                        else:
        ...
        if err_:
            for i in range(len(err_)):
                err.push(err_[i][0], var_name=err_[i][1], given=err_[i][2], expected=err_[i][3], function_name="_input_parameter_compile_processing")
            err.pop()
            return None
