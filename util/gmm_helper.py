...
class GmmMml(TransformerMixin):
    ...
    def _draw_elipse(self, ax, estcov, estmu):
        ...
        ell = mpl.patches.Ellipse(estmu, v[0], v[1], 180 + angle, facecolor="none", edgecolor="black")
    ...
    def _plot_graph(self, estcov, estmu, k, y, message):
        if y.shape[1] == 2:
            ...
            if self.plot_dir is not None:
                plt.savefig(Path(self.plot_dir, f"fig_{int(time.time())}.png"))
            plt.show()
        else:
        ...
        while k_cont == True:
            ...
            while cont == True or iteration < self.max_iters:
                ...
                while comp < k:
                    ...
                    normindic = np.divide(
                        indic,
                        (np.finfo(np.float64).tiny + np.kron(np.ones((k, 1)), np.sum(indic, axis=0))),
                    )
                ...
                dlength = (
                    -loglike[countf] + (nparsover2 * np.sum(np.log(estpp))) + (nparsover2 + 0.5) * k * np.log(npoints)
                )
                dl.append(dlength)
                kappas.append(k)
